...
namespace Nano::Refl::Detail
{
    template<typename C, C... cs>struct TStr
    {
        ...
        using Char = C; 
        static constexpr char nameData[]{cs...,C(0)}; 
        static constexpr std::string_view name{nameData}; 
    };
    ...
    constexpr size_t FindIf(L, F&&, std::index_sequence<>) { return -1; }
    ...
    {
        if constexpr (!m0) 
            return Acc<ms...>(l, std::forward<F>(f), std::forward<R>(r), std::index_sequence<Ns...>{});
        else 
            return Acc<ms...>(l, std::forward<F>(f), f(std::forward<R>(r), l.template Get<N0>()), std::index_sequence<Ns...>{});
    }
...
namespace Nano::Refl
{
    ...
    struct NameValuePair<Name, void> : Name
    {
        static constexpr bool hasValue = false;
        template<typename U> constexpr bool operator==(U) const { return false; }
    };

    template<typename...T> struct ElemList 
    {
        std::tuple<T...> elements; 
        static constexpr size_t size = sizeof...(T);
        constexpr ElemList(T... elems) : elements{ elems... } {}
        ...
        template<class E>
        constexpr auto PushBack(E e) const
        {
            return std::apply([e](auto&&... es)
                {
                    return ElemList<T..., E>{es...,e};
                },
                elements
            );
        }
        ...
        {
            if constexpr(std::is_same_v<T,E> || ...) //why?
                return *this;
            else 
                return PushBack(e);
        }
